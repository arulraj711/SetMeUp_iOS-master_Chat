...
- (void) setup
{
    ...
    //  pageCnt = 1;
    UITapGestureRecognizer* singleTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(touchToScroll)];
...
-(void) touchToScroll {
    //NSLog(@"pagename:%@",_pageName);
    //NSLog(@"page count:%d",pageCnt);
    if([_pageName isEqualToString:@"AppTutorial"]&&pageCnt<2) {
        float offset = pageCnt+1.0;
        //NSLog(@"int offset:%d",(int)offset);
        self.pageControl.currentPage = (int)offset;
        self.pageControl1.currentPage = (int)offset;
        [self crossDissolveForOffset:offset];
        pageCnt++;
    } else if([_pageName isEqualToString:@"UserCTutorial"]&&pageCnt<1) {
        float offset = pageCnt+1.0;
        self.pageControl.currentPage = (int)offset;
        self.pageControl1.currentPage = (int)offset;
        [self crossDissolveForOffset:offset];
        pageCnt++;
    } else if([_pageName isEqualToString:@"FCTutorial"]&&pageCnt<1) {
        float offset = pageCnt+1.0;
        self.pageControl.currentPage = (int)offset;
        self.pageControl1.currentPage = (int)offset;
        [self crossDissolveForOffset:offset];
...
//        [self crossDissolveForOffset:offset];
        ...
        [UIView animateWithDuration:0.3 animations:^{
            ...
            if([_pageName isEqualToString:@"FCTutorial"]) {
                [shRes showProgressHUDForView];
                [shRes fetchConnUser];
            }
...
- (void) orientFooter
{
    if (self.walkThroughDirection == GHWalkThroughViewDirectionVertical) {
        ...
        BOOL isRotated1 = !CGAffineTransformEqualToTransform(self.pageControl1.transform, CGAffineTransformIdentity);
        if (!isRotated1) {
            //            CGRect butonFrame = self.skipButton.frame;
            //            butonFrame.origin.x -= 30;
            //            self.skipButton.frame = butonFrame;
            self.pageControl1.transform = CGAffineTransformRotate(self.pageControl1.transform, M_PI_2);
            CGRect frame1 = self.pageControl1.frame;
            frame1.size.height = ([self.dataSource numberOfPages] + 1 ) * 16;
            frame1.origin.x = 10;
            frame1.origin.y = self.frame.size.height/2-frame1.size.height;
            self.pageControl1.frame = frame1;
        }
    } else{
        BOOL isRotated = !CGAffineTransformEqualToTransform(self.pageControl.transform, CGAffineTransformIdentity);
        if (isRotated) {
...
- (void)buildFooterView {
    self.pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, self.frame.size.height/2, self.frame.size.width, 20)];
    ...
    self.pageControl1 = [[UIPageControl alloc] initWithFrame:CGRectMake(0, self.frame.size.height/2, self.frame.size.width, 20)];
    
    //Set defersCurrentPageDisplay to YES to prevent page control jerking when switching pages with page control. This prevents page control from instant change of page indication.
    self.pageControl1.defersCurrentPageDisplay = YES;
    self.pageControl1.pageIndicatorTintColor = [UIColor lightGrayColor];
    self.pageControl1.currentPageIndicatorTintColor = [UIColor whiteColor];
    self.pageControl1.autoresizingMask =  UIViewAutoresizingFlexibleWidth;
    [self.pageControl addTarget:self action:@selector(showPanelAtPageControl:) forControlEvents:UIControlEventValueChanged];
    [self addSubview:self.pageControl1];
    [self bringSubviewToFront:self.pageControl1];
    ...
    self.skipButton.frame = CGRectMake(-10,20, 80, 30);
    ...
    self.doneButton.frame = CGRectMake(0,self.frame.size.height-40, 320, 40);
    
    self.doneButton.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin;
    [self.doneButton setImage:[UIImage imageNamed:@"tut_done_btn"] forState:UIControlStateNormal];
    //[self.doneButton setTitle:@"Done" forState:UIControlStateNormal];
    [self.doneButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [self.doneButton addTarget:self action:@selector(skipIntroduction) forControlEvents:UIControlEventTouchUpInside];
...
- (void) showPanelAtPageControl:(UIPageControl*) sender
{
    ...
    [self.pageControl1 setCurrentPage:sender.currentPage];
}
...
- (void) showInView:(UIView *)view animateDuration:(CGFloat) duration
{
    ...
    self.pageControl1.numberOfPages = [self.dataSource numberOfPages];
