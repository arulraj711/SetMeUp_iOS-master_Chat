//
//  ITSideMenuContainerViewController.m
...
NSString * const ITSideMenuStateNotificationEvent = @"ITSideMenuStateNotificationEvent";

typedef enum {
    ITSideMenuPanDirectionNone,
    ITSideMenuPanDirectionLeft,
    ITSideMenuPanDirectionRight
} ITSideMenuPanDirection;

@interface ITSideMenuContainerViewController ()
@property (nonatomic, strong) UIView *menuContainerView;

@property (nonatomic, assign) CGPoint panGestureOrigin;
@property (nonatomic, assign) CGFloat panGestureVelocity;
@property (nonatomic, assign) ITSideMenuPanDirection panDirection;

@property (nonatomic, assign) BOOL viewHasAppeared;
@end

@implementation ITSideMenuContainerViewController
...
+ (ITSideMenuContainerViewController *)containerWithCenterViewController:(id)centerViewController
    ...
    ITSideMenuContainerViewController *controller = [ITSideMenuContainerViewController new];
...
- (void)setDefaultSettings {
    ...
    self.menuState = ITSideMenuStateClosed;
    ...
    self.panMode = ITSideMenuPanModeDefault;
...
- (void)setCenterViewController:(UIViewController *)centerViewController {
    ...
    self.shadow = [ITSideMenuShadow shadowWithView:[_centerViewController view]];
    [self.shadow draw];
...
- (void)toggleLeftSideMenuCompletion:(void (^)(void))completion {
    if(self.menuState == ITSideMenuStateLeftMenuOpen) {
        [self setMenuState:ITSideMenuStateClosed completion:completion];
    } else {
        [self setMenuState:ITSideMenuStateLeftMenuOpen completion:completion];
    }
}

- (void)toggleRightSideMenuCompletion:(void (^)(void))completion {
    if(self.menuState == ITSideMenuStateRightMenuOpen) {
        [self setMenuState:ITSideMenuStateClosed completion:completion];
    } else {
        [self setMenuState:ITSideMenuStateRightMenuOpen completion:completion];
    }
}
...
- (void)setMenuState:(ITSideMenuState)menuState {
...
- (void)setMenuState:(ITSideMenuState)menuState completion:(void (^)(void))completion {
    void (^innerCompletion)() = ^ {
        ...
        ITSideMenuStateEvent eventType = (_menuState == ITSideMenuStateClosed) ? ITSideMenuStateEventMenuDidClose : ITSideMenuStateEventMenuDidOpen;
    ...
    switch (menuState) {
        case ITSideMenuStateClosed: {
            [self sendStateEventNotification:ITSideMenuStateEventMenuWillClose];
        ...
        case ITSideMenuStateLeftMenuOpen:
            if(!self.leftMenuViewController) return;
            [self sendStateEventNotification:ITSideMenuStateEventMenuWillOpen];
            [self leftMenuWillShow];
            [self openLeftSideMenuCompletion:innerCompletion];
            break;
        case ITSideMenuStateRightMenuOpen:
            if(!self.rightMenuViewController) return;
            [self sendStateEventNotification:ITSideMenuStateEventMenuWillOpen];
...
- (void)sendStateEventNotification:(ITSideMenuStateEvent)event {
    NSDictionary *userInfo = [NSDictionary dictionaryWithObject:[NSNumber numberWithInt:event]
                                                         forKey:@"eventType"];
    [[NSNotificationCenter defaultCenter] postNotificationName:ITSideMenuStateNotificationEvent
                                                        object:self
                                                      userInfo:userInfo];
}
...
#pragma mark -
#pragma mark - ITSideMenuPanMode

- (BOOL) centerViewControllerPanEnabled {
    return ((self.panMode & ITSideMenuPanModeCenterViewController) == ITSideMenuPanModeCenterViewController);
}

- (BOOL) sideMenuPanEnabled {
    return ((self.panMode & ITSideMenuPanModeSideMenu) == ITSideMenuPanModeSideMenu);
}
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch {
    if([gestureRecognizer isKindOfClass:[UITapGestureRecognizer class]] &&
       self.menuState != ITSideMenuStateClosed) return YES;
...
- (void) handlePan:(UIPanGestureRecognizer *)recognizer {
	...
	if(recognizer.state == UIGestureRecognizerStateBegan) {
        ...
        self.panDirection = ITSideMenuPanDirectionNone;
	}
    
    if(self.panDirection == ITSideMenuPanDirectionNone) {
        CGPoint translatedPoint = [recognizer translationInView:view];
        if(translatedPoint.x > 0) {
            self.panDirection = ITSideMenuPanDirectionRight;
            if(self.leftMenuViewController && self.menuState == ITSideMenuStateClosed) {
                [self leftMenuWillShow];
            }
        }
        else if(translatedPoint.x < 0) {
            self.panDirection = ITSideMenuPanDirectionLeft;
            if(self.rightMenuViewController && self.menuState == ITSideMenuStateClosed) {
                [self rightMenuWillShow];
            }
        }
    }
    
    if((self.menuState == ITSideMenuStateRightMenuOpen && self.panDirection == ITSideMenuPanDirectionLeft)
       || (self.menuState == ITSideMenuStateLeftMenuOpen && self.panDirection == ITSideMenuPanDirectionRight)) {
        self.panDirection = ITSideMenuPanDirectionNone;
        return;
    }
    
    if(self.panDirection == ITSideMenuPanDirectionLeft) {
        [self handleLeftPan:recognizer];
    } else if(self.panDirection == ITSideMenuPanDirectionRight) {
        [self handleRightPan:recognizer];
    }
}

- (void) handleRightPan:(UIPanGestureRecognizer *)recognizer {
    if(!self.leftMenuViewController && self.menuState == ITSideMenuStateClosed) return;
    ...
    if(recognizer.state == UIGestureRecognizerStateEnded) {
        ...
        if(self.menuState == ITSideMenuStateClosed) {
            ...
            if(hideMenu) {
                self.panGestureVelocity = velocity.x;
                [self setMenuState:ITSideMenuStateClosed];
            } else {
                self.panGestureVelocity = 0;
...
- (void) handleLeftPan:(UIPanGestureRecognizer *)recognizer {
    if(!self.rightMenuViewController && self.menuState == ITSideMenuStateClosed) return;
	...
	if(recognizer.state == UIGestureRecognizerStateEnded) {
        ...
        if(self.menuState == ITSideMenuStateClosed) {
            ...
            if(showMenu) {
                self.panGestureVelocity = velocity.x;
                [self setMenuState:ITSideMenuStateRightMenuOpen];
            } else {
                self.panGestureVelocity = 0;
                [self setCenterViewControllerOffset:0 animated:YES completion:nil];
            }
...
- (void)setUserInteractionStateForCenterViewController {
    ...
    if([self.centerViewController respondsToSelector:@selector(viewControllers)]) {
        ...
        for(UIViewController* viewController in viewControllers) {
            viewController.view.userInteractionEnabled = (self.menuState == ITSideMenuStateClosed);
        }
    }
    // disable interactivepopGesture of center viewcontroller (navigation type only) if the menu is visible
    if ([self.centerViewController respondsToSelector:@selector(interactivePopGestureRecognizer)]) {
        UINavigationController *navController=(UINavigationController*)self.centerViewController;
        if (self.menuState==ITSideMenuStateClosed) {
