...
+ (SPGooglePlacesAutocompletePlace *)placeFromDictionary:(NSDictionary *)placeDictionary {
    ...
    NSString *functionName=[[NSUserDefaults standardUserDefaults] objectForKey:@"functionName"];
    if([functionName isEqualToString:@"autoSearch"]) {
        place.name = [placeDictionary objectForKey:@"description"];
    } else if([functionName isEqualToString:@"defaultSearch"]){
